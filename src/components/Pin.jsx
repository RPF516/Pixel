  ...
  return (
    <div className="m-2">
      ...
      >
        ...
        {postHovered && (
          <div
          className="absolute top-0 w-full h-full w-flex flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50"
          style={{ height: '100%' }}
          >
            <div
            ...
            <div className="flex justify-between items-center gap-2 w-full">
              {destination && (
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-white flex items-center gap-2 text-black font-bold p-2 pl-2 pr-2 pt-1 pb-1 rounded-full opacity-70 hover:100 hover:shadow-md"
                >
      ...
      <Link to={`user-profile/${postedBy?._id}`} className="flex gap-2 mt-2 items-center transform hover:scale-105 hover:shadow-md">
        <img
        className="w-6 h-6 rounded-full object-cover"
        src={postedBy?.image}
        alt="user-profile"
        />
        <p className="text-gray-800 font-semibold capitalize p-1 hover:text-gray-900">{postedBy?.userName}</p>
